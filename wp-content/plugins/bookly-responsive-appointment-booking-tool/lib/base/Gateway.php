...
abstract class Gateway
{
    ...
    public function createIntent()
    {
        if ( $this->createPayment() ) {
            ...
            $data = array();
            if ( $this->request->getCartInfo()->getPayNow() > 0 ) {
                $data = $this->createGatewayIntent();
            } else {
                $coupon = $this->request->getUserData()->getCoupon();
                if ( $coupon ) {
                    Booking\Proxy\Coupons::claim( $coupon->getId() );
                }
            }
    ...
    public function complete()
    {
        ...
        if ( $payment ) {
            if ( $payment->getStatus() !== Entities\Payment::STATUS_COMPLETED ) {
                ...
                if ( ! in_array( $payment->getType(), array( Entities\Payment::TYPE_LOCAL, Entities\Payment::TYPE_FREE ), true ) ) {
        ...
        if ( $required_sync ) {
            if ( $payment && $payment->getTarget() === Entities\Payment::TARGET_GIFT_CARDS ) {
                Payment\Proxy\Pro::completeGiftCard( $payment, $this->request->getUserData()->getCustomer() );
            } else {
