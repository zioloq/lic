...
//        if ( strpos($_SERVER['REQUEST_URI'],'page=wpbc-new') !== false ) { $booking_is_days_always_available = 'On'; }
        // if ( in_array( $resource_id, array( '12', '15', '17' ) ) ) $booking_is_days_always_available = 'On';     // Set  dates in calendar always available only  for specific resources with specific ID
        ...
        if ($my_selected_dates_without_calendar == '') {
            ...
            if ($start_month_calendar !== false)
                if (is_array($start_month_calendar))
	                $start_js_month = ", [" . strval( intval( $start_month_calendar[0] ) ) . "," . strval( intval( $start_month_calendar[1] ) ) . "] ";     //FixIn: 9.6.1.1
    ...
    function pre_get_calendar_html( $resource_id=1, $cal_count=1, $bk_otions=array() ){
        ...
        if ( (!class_exists('wpdev_bk_personal')) && ($booking_is_show_powered_by_notice == 'On') )
            $calendar .= '<div style="font-size:10px;text-align:left;margin:5px 0 15px;text-shadow: none;">Powered by <a href="https://wpbookingcalendar.com" target="_blank" title="Booking Calendar plugin for WordPress">Booking Calendar</a></div>';
